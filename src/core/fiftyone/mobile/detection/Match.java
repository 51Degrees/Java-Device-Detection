 ...
 * accessor provided with a Property or the string name of the property.
 * <p>
 * The Signature the target device match against can be returned 
...
public class Match {
	 ...
	 */
    private class SignatureIndexFrequencies
    {
        SignatureIndexFrequency first;

        SignatureIndexFrequency last;

        void addBefore(SignatureIndexFrequency existing, SignatureIndexFrequency newItem)
        {
        ...
        void addAfter(SignatureIndexFrequency existing, SignatureIndexFrequency newItem)
        {
         ...
         */
        void add(SignatureIndexFrequency newItem)
        {
            if (last != null)
         ...
         */
        void remove(SignatureIndexFrequency existing)
        {
            if (first == existing)
                first = existing.next;
            if (last == existing)
	 ...
	 */
	private class SignatureIndexFrequency {
		 ...
		 */
        public SignatureIndexFrequency next;

        /**
         * The previous signature index in the linked list.
         */
        public SignatureIndexFrequency previous;
        
		SignatureIndexFrequency(int signatureIndex, int frequency) {
	...
	/**
	 * Comparator used to order the signatures by rank.
	 */
	private static final Comparator<Signature> signatureRankComparator = new Comparator<Signature>() {
	...
	private static final Comparator<Node> nodeComparator = new Comparator<Node>() {
		public int compare(Node o1, Node o2) {
			int l0 = o1.getSignatureIndexes().length;
			int l1 = o2.getSignatureIndexes().length;
			if (l0 < l1)
				return -1;
			if (l0 > l1)
				return 1;
	...
	/**
	 * @return if the nodes of the match correspond exactly to a signature 
	 * then return the index of the signature. Otherwise -1.
	 * @throws IOException 
	 */
	 ...
	 * @throws IOException 
	 */
	List<Signature> getClosestSignatures() throws IOException {
		...
		SignatureIndexFrequencies linkedList = buildInitialList(current.getSignatureIndexes());
        
        // Count the number of times each signature index occurs.
        while (nodeIterator.hasNext()) {
        	current = nodeIterator.next();
            maxCount = getClosestSignaturesForNode(
            		current.getSignatureIndexes(),
            		linkedList,
            		maxCount,
            		iteration);
            iteration++;
        ...
        return getClosestSignaturesFinal(maxCount, linkedList);
	}

	/**
	 * Convert the signature indexes to the signature objects from
	 * the data set list of signatures and order them by the rank
	 * so that the most popular signatures are checked first.
	 * @param maxCount frequency of indexes required
	 * @param linkedList linked list of indexes
	 * @return a list of fully loaded signatures
	 * @throws IOException
	 */
	private List<Signature> getClosestSignaturesFinal(
			int maxCount,
			SignatureIndexFrequencies linkedList) throws IOException {
        SignatureIndexFrequency current = linkedList.first;
		while(current != null)
		{
			if (current.frequency == maxCount) {
				Signature signature = getDataSet().getSignatures().get(current.signatureIndex);
				signaturesRead++;
				signatures.add(
						~Collections.binarySearch(signatures, signature, signatureRankComparator),
						signature);
			}
	...
	private int getClosestSignaturesForNode(
			...
			SignatureIndexFrequencies linkedList, 
		...
		while (signatureIndex < signatureIndexList.length && current != null) {
		    if (current.signatureIndex > signatureIndexList[signatureIndex]) {
		        ...
		        if (thresholdReached == false)
		        	linkedList.addBefore(
		        		current,
		                new SignatureIndexFrequency(
		                		signatureIndexList[signatureIndex], 
		                		1));
		        signatureIndex++;
		    }
		    else if (current.signatureIndex < signatureIndexList[signatureIndex]) {
		        if (thresholdReached) {
                    // Threshold reached so we can removed this item
                    // from the list as it's not relevant.
                    SignatureIndexFrequency nextItem = current.next;
                    if (current.frequency < maxCount)
	...
	private SignatureIndexFrequencies buildInitialList(int[] list) {
		SignatureIndexFrequencies linkedList = new SignatureIndexFrequencies();
        for(int index : list)
        	linkedList.add(new SignatureIndexFrequency(index, 1));
        return linkedList;
	}
     ...
     * @return Array of the values associated with the property, or null 
    ...
    /**
     * Replaces any characters in the target user agent which are outside
     * the range the dataset used when it was built with question marks.
     */
