 ...
 * <p>
 * A signature contains those characters of a user agent which are relevant for the
 ...
 * are very relevant.
 * <p>
 * A signature contains both an array of relevant characters from user agents identified
 * when the data was created and the unique complete node identifies of relevant sub strings
 ...
 * <p>
 * Unlike other entities the signature may have a varying number of nodes and profiles
 * associated with it depending on the data set. All signatures within a data set will
 * have the same number of profiles and nodes associated with them all. As these can change
 * across data sets they can't be included in the source code. As such a secondary header
 * is used in the data set to indicate the number of profiles and nodes in use. The 
 * Memory.SignatureList and Stream.SignatureList lists are 
...
public class Signature extends BaseEntity implements Comparable<Signature> {
	...
	/**
	 * The relative rank of the signature compared to other signatures where 1
	 * is the most popular signature. The rank property of signature will
	 * replace the Popularity property in the future.
	 */
	public int Rank() {
		return rank;
	}
	 ...
	 * @param index
	 *            The index in the data structure to the node
	 * @param reader
	 ...
	 * 
	 * @param nodeOffsets
	 *            The node indexes to be compared against
