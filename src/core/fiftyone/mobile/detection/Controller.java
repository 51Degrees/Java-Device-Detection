 ...
 * nodes relate exactly to an existing signature. Any popular device will be
 * found at this point. The approach is exceptionally fast at identifying
 * popular devices.
 * <p>
 * Step 3 - If the target user agent is less popular, or newer than the creation
 * time of the data set, a small sub set of possible signatures are identified
 * from the matching nodes. These signatures are evaluated against the target
 * user agent to determine the different in relevant characters between them.
 * The signature which has the lowest difference and is most popular is then
 * returned.
 * <p>
 ...
 * the result match. Older data sets that are unaware of the latest devices, or
 * user agent formats in use will be less accurate.
 * <p>
...
public class Controller {
	 ...
	 * <p>
	 * The memory implementation of the data set will always perform fastest but
	...
	public void match(Match match) throws IOException {
		...
		if (match.getTargetUserAgentArray().length == 0 ||
            ...
            else
            {
                ...
                {
                    List<Signature> closestSignatures = match.getClosestSignatures();
